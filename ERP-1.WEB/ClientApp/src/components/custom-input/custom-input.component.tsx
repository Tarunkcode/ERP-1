...
export function CustomSelect({ label, name, dataArray, handleChange, classCategory, ...otherProps }: any) {
    ...
    return (
        <span className="row row-content d-flex section2 col-sm-12 m-0">
            ...
            <label htmlFor={name} style={{ fontSize: '1rem' }} className="form-label labl ml-2 mr-2 labl2">{label}</label>
            {/*<label htmlFor={name} style={{ fontSize: '1rem' }} className="form-label mr-2 col-4">{label}</label>*/}
...
export function DownShift({ params, ...rest} : any) {
    const items = [
                ...
                <div>
                    ...
                    >
                        <input
                            ...
                            className="w-full p-1.5 ag-grid-auto"
                            style={{ height: '30px', width: "100%",padding: '18px 8px', border:'none'}}
                            {...getInputProps()}
                        />
