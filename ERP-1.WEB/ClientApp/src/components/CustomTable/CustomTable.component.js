...
function WriteTable(_a) {
    ...
    return (React.createElement("div", { className: "card-body m-0 p-0 mb-1 table-responsive", style: { margin: "0", padding: "0", overflow: 'auto' } },
        ...
        React.createElement("table", { className: "table table-responsive table-striped table-bordered table-sm", style: { width: '100%' } },
            React.createElement("thead", null,
                React.createElement("tr", null,
                    React.createElement("th", null, "S.No."),
                    columns.map(function (head) {
                        return (React.createElement("th", { className: "text-center", style: { fontWeight: 400, backgroundColor: 'lightslategray', color: 'white', fontSize: '0.925rem' } }, head.header));
                    }),
                ...
                return (React.createElement("tr", { key: ind, id: ind },
                    ...
                    columns.map(function (col) {
                        return (React.createElement("td", null,
                            React.createElement(custom_input_component_1.InputList, { row: ind, name: row[col.field].id, change: HandleIpSelect, width: row[col.field].width, ipType: row[col.field].ipType || "text", type: row[col.field].typeBox, ipTitle: row[col.field].ipTitle, dataArray: row[col.field].dataArray, classCategory: row[col.field].classCat, placeholder: row[col.field].placeholder, s: "100%", id: row[col.field].name, default: row[col.field].dataArray.length !== 0 && row[col.field].defaultList.length !== 0 ? row[col.field].dataArray.findIndex(function (x) { return x.code == row[col.field].defaultList[ind][row[col.field].name]; }) : undefined })));
                        //return ( <td>{row[col.field]}</td>)
                    }),
                    ind === dataArr.length - 1 ? (React.createElement("td", null,
                        React.createElement("button", { type: "button", className: "btn btn-outline-danger m-0 p-1 pr-2 pl-2", onClick: function () { return deleteRowFunc(ind, dataArr, setRowFunc); } }, "x"))) : React.createElement("td", null)));
            })))));
}
