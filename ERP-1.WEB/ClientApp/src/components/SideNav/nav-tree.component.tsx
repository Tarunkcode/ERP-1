...
//child
function TreeNode({ node, key }: any) {
        ...
        <>
            ...
            <ul>
                <li className="ram" id={Key} style={{ backgroundColor: '#24509e', fontWeight: 600, color: 'black' }}>
                    ...
                    {
                        type === '3' ? (
                            address === '/modify' ? ((<NavLink className="p-1 m-0 abascus" to={{ pathname: address + `/${Mt}`, state: { mAdd: `${Maddress}`, mT: `${Mt}` } }}><span className=" ml-5 nav-span">{label}</span></NavLink>)) : (<NavLink className="p-1 m-0 abascus" to={address}><span className=" ml-5 nav-span">{label}</span></NavLink>)
                        ) : null
