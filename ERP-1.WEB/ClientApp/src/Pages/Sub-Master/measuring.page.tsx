import * as React from 'react';
import CustomInput, { MasterInput2 } from '../../components/custom-input/custom-input.component';

export default function Measuring ({ defaultData, getMasterType, pageTitle, configType, handleChange, handlePosting }: any) {
    React.useEffect(() => {
        configType == '31' ? getMasterType(1020) : null;
    }, [defaultData])
    return (
        <>
            <div className="main card firstDiv">
                <div
                    className="text-center card-title col-12"
                    style={{ textAlign: "start", backgroundColor: "#8389d4" }}
                >
                    <span className="row-header p-0 m-0">{pageTitle}</span>
                </div>
                <form className="row-content form col-sm-12 pt-0">

                    <span className="d-flex section2 mb-2 col-sm-12">
                        <MasterInput2
                            name="d1"
                            handleChange={handleChange}
                            defaultt={defaultData.d1}
                            classCategory="form-control col-12 subMaster"
                            ipType="number"
                            label="Range"
                            ipTitle="Enter Decsription"
                        
                        />

                    </span>

                    <span className="d-flex section2 mb-2 col-sm-12">
                        <label htmlFor="name" style={{ fontSize: '1em' }} className="form-label labl labl2 mr-2 ml-2"></label>
                        <textarea name="name" onChange={handleChange} defaultValue={defaultData.name} placeholder="Enter Description" rows={1} cols={10} style={{borderColor:'#86a4c3'}} className="form-control col-10 subMaster" />
                    </span>

                </form>
            <div className="btn-group col-12 mt-3" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', }}>
                <button type="button" style={{ border: '2px solid green', letterSpacing: 3, width: "100px" }} className="btn btn-success pl-0 pr-0 col-xs-6 col-md-1" onClick={handlePosting} >Save</button>

            </div>
            </div>
        </>
    )
}